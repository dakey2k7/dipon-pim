...
export function getDb(): Database.Database {
  ...
  // === TEA Cleanup: Triethanolamin auf exakt 2 Lieferanten (Brenntag + Reininghaus) normalisieren ===
  try {
    const matBRE = _db.prepare(`SELECT id FROM materials WHERE code IN ('TEA99-BRE','TEA99-REI-MERGED') AND is_active=1`).get() as any
    const matREI = _db.prepare(`SELECT id FROM materials WHERE code IN ('TEA99-REI','TEA99-REI-MERGED') AND id != ?`, ).get

    // Finde TEA99-BRE (aktives Material)
    const breMat = _db.prepare(`SELECT id FROM materials WHERE code='TEA99-BRE' AND is_active=1`).get() as any
    const reiMat = _db.prepare(`SELECT id FROM materials WHERE code LIKE 'TEA99-REI%'`).get() as any

    if (breMat) {
      // 1. Alle product_materials auf TEA99-BRE umleiten
      if (reiMat) {
        _db.prepare(`UPDATE product_materials SET material_id=? WHERE material_id=?`).run(breMat.id, reiMat.id)
        _db.prepare(`UPDATE materials SET is_active=0 WHERE id=?`).run(reiMat.id)
      }

      // 2. Exakt die richtigen Lieferanten ermitteln
      const brenntag   = _db.prepare(`SELECT id FROM suppliers WHERE name='Brenntag' LIMIT 1`).get() as any
      const reininghaus = _db.prepare(`SELECT id FROM suppliers WHERE name='Reininghaus Chemie' LIMIT 1`).get() as any

      // 3. Falsche Lieferanten (z.B. Biesterfeld) aus TEA99-BRE supplier_prices entfernen
      const allowedIds = [brenntag?.id, reininghaus?.id].filter(Boolean)
      if (allowedIds.length > 0) {
        _db.prepare(
          `DELETE FROM supplier_prices WHERE material_id=? AND supplier_id NOT IN (${allowedIds.map(()=>'?').join(',')})`
        ).run(breMat.id, ...allowedIds)
      }

      // 4. Duplikate bereinigen (behalte den neuesten je Lieferant)
      _db.exec(`
        ...
        ) AND material_id=${breMat.id}
      `)

      // 5. Reininghaus mit korrektem Preis 1.20 €/kg hinzufügen falls fehlend
      if (reininghaus) {
        const exists = _db.prepare(`SELECT id FROM supplier_prices WHERE material_id=? AND supplier_id=?`).get(breMat.id, reininghaus.id)
        if (!exists) {
          _db.prepare(`INSERT INTO supplier_prices (material_id,supplier_id,price_per_unit,unit,currency,valid_from,is_preferred) VALUES (?,?,1.20,'kg','EUR',date('now'),0)`).run(breMat.id, reininghaus.id)
        }
      }

      // 6. Brenntag als preferred setzen (günstigster)
      if (brenntag) {
        _db.prepare(`UPDATE supplier_prices SET is_preferred=0 WHERE material_id=?`).run(breMat.id)
        _db.prepare(`UPDATE supplier_prices SET is_preferred=1 WHERE material_id=? AND supplier_id=?`).run(breMat.id, brenntag.id)
      }
    }
