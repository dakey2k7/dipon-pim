...
export function registerMaterialHandlers(): void {
  ...
  ipcMain.handle('materials:update', (_e, id: number, d: Record<string, unknown>) => {
    ...
    // Lieferant-Wechsel: is_preferred in supplier_prices synchronisieren
    if (d.supplier_id) {
      const supplierId = Number(d.supplier_id)
      const existing = db.prepare(`SELECT id FROM supplier_prices WHERE material_id=? AND supplier_id=?`).get(id, supplierId)
      if (existing) {
